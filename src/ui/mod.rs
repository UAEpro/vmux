...
#[cfg(test)]
use command_palette::{
    command_palette_entries, command_palette_lines, palette_shortcut, prefix_action_bindings,
};
use command_palette::{
    draw_commands, filter_command_entries, prefix_key_action, CommandPaletteAction,
    CommandPaletteEntry,
};
mod theme;
...
impl Ui {
        ...
        match event {
            ...
            Event::Mouse(mouse) => match mouse.kind {
                ...
                MouseEventKind::Up(MouseButton::Left) => {
                    ...
                    {
                        // Clear ALL in-progress drag flags — leaving
                        // sidebar_resize_drag set makes the next in-pane drag
                        // resize the sidebar instead of selecting text.
            ...
            Event::Resize(_, _) => {
                ...
                // View height changed — scroll max may shrink.
                self.clamp_all_scroll_offsets();
            }
            _ => {}
        }
    ...
    /// Max lines the user may scroll up for a pane (content − view height).
    fn max_scroll_for_pane(&self, pane_id: &str) -> usize {
        let Some(snapshot) = self.snapshot.as_ref() else {
            return 0;
        };
        ...
        max_pane_scroll_offset(pane, self.pane_content_height(pane_id))
    }

    fn pane_content_height(&self, pane_id: &str) -> usize {
        let has_strip = self
            .snapshot
            .as_ref()
            ...
            .unwrap_or(false);
        self.pane_area_by_id(pane_id)
            .ok()
            .flatten()
            .map(|area| usize::from(pane_content_area(area, has_strip).height.max(1)))
            .unwrap_or(24)
            .max(1)
    }

    /// Drop or shrink scroll offsets that exceed the current scrollback window.
    fn clamp_all_scroll_offsets(&mut self) {
        ...
        for (pane_id, offset) in &self.scroll_offsets {
            let max = snapshot
                .panes
                .get(pane_id)
                .map(|pane| max_pane_scroll_offset(pane, self.pane_content_height(pane_id)))
                .unwrap_or(0);
            if *offset > max {
                stale.push((pane_id.clone(), max));
            }
        }
        for (pane_id, max) in stale {
            if max == 0 {
                self.scroll_offsets.remove(&pane_id);
            } else {
                self.scroll_offsets.insert(pane_id, max);
            }
        }
    }
    ...
    /// True while a confirm/rename modal owns input even though `self.mode` is
    /// still `Panes`. Pane input (keys, mouse, wheel) must not reach the pane
    /// underneath while one of these is open.
    fn modal_active(&self) -> bool {
        self.pending_confirm.is_some() || self.rename_dialog.is_some()
    }
    ...
    fn handle_context_click(&mut self, area: Rect, column: u16, row: u16) -> Result<bool> {
        ...
        // Row `area.y` is the top border; entries start on the next row.
        let rel = row.saturating_sub(area.y);
        if rel == 0 {
            return Ok(false);
        }
        let index = usize::from(rel - 1);
...
/// spaces (e.g. `" commands "`).
pub(crate) fn panel_block(title: &str, palette: ThemePalette) -> Block<'static> {
    Block::default()
        .title(Span::styled(
            title.to_string(),
            Style::default()
...
pub(crate) fn selected_row_style(palette: ThemePalette) -> Style {
    Style::default()
...
fn confirm_dialog_bg(theme: UiTheme) -> Color {
    // Slightly lifted surface so the modal reads above the pane grid on every theme.
...
/// How far (in lines from the live bottom) a pane may be scrolled.
...
fn adjust_scroll(offsets: &mut BTreeMap<String, usize>, pane: &str, delta: isize, max: usize) {
    if max == 0 {
        offsets.remove(pane);
        return;
    }
    let current = offsets.get(pane).copied().unwrap_or(0).min(max);
    let next = if delta.is_negative() {
        // Negative delta = scroll toward live bottom (decrease offset).
        current.saturating_sub(delta.unsigned_abs())
    } else {
    ...
    if next == 0 {
        ...
        offsets.insert(pane.to_string(), next);
    }
}
...
fn pane_text_for_view(pane: &crate::model::Pane, height: usize, scroll_offset: usize) -> String {
    ...
    let max = lines.len().saturating_sub(height.max(1));
    let scroll_offset = scroll_offset.min(max);
    if scroll_offset == 0 {
        return tail_lines(&pane.output, height.max(1));
    }
...
fn selected_text_from_pane(
    ...
    // Match the renderer's window exactly. At offset 0 pane_text_for_view
    // bottom-aligns via tail_lines (start = len - height), so mapping content
    // rows from the top of the buffer copies the wrong lines once output exceeds
    // the view height. (For a buffer that fits, len - height saturates to 0, so
    // simple selections are unchanged.)
...
fn notification_workspace<'a>(
    ...
    // Check every tab, not just the active-tab live view, so a notification from
    // a pane on a background tab can still be located and jumped to.
    snapshot
        .workspaces
        .iter()
...
pub(crate) fn trim_label(value: &str, max: usize) -> String {
...
pub(crate) fn truncate_to_width(value: &str, max: usize) -> String {
    if max == 0 {
        return String::new();
    }
...
mod tests {
    ...
    fn ui_theme_catalog_has_fifteen_named_themes() {
        ...
        for name in &names {
            assert!(
                crate::config::supported_themes().contains(name),
                "theme {name} missing from config::supported_themes"
            );
    ...
    fn adjusts_scroll_offsets_with_bounds() {
        ...
        adjust_scroll(&mut offsets, "pane-1", 3, 100);
        assert_eq!(offsets.get("pane-1"), Some(&3));
        adjust_scroll(&mut offsets, "pane-1", -2, 100);
        assert_eq!(offsets.get("pane-1"), Some(&1));
        adjust_scroll(&mut offsets, "pane-1", -2, 100);
        assert_eq!(offsets.get("pane-1"), None);
        // Cannot scroll past content (max = view-limited history).
        adjust_scroll(&mut offsets, "pane-1", 50, 10);
        assert_eq!(offsets.get("pane-1"), Some(&10));
        adjust_scroll(&mut offsets, "pane-1", 5, 10);
        assert_eq!(offsets.get("pane-1"), Some(&10));
        // Empty / short content: no scroll.
        adjust_scroll(&mut offsets, "pane-1", 5, 0);
        assert_eq!(offsets.get("pane-1"), None);
    }

    #[test]
    fn max_pane_scroll_offset_matches_content_window() {
        let mut pane = crate::model::Pane::new("p".into(), "sh".into(), SplitDirection::Right);
        pane.scrollback = "a\nb\nc\nd\ne\n".into();
        // 5 lines, view height 3 → can scroll up by 2.
        assert_eq!(max_pane_scroll_offset(&pane, 3), 2);
        // View taller than content → no scroll.
        assert_eq!(max_pane_scroll_offset(&pane, 10), 0);
        pane.scrollback.clear();
        pane.output = "only\ntwo\n".into();
        assert_eq!(max_pane_scroll_offset(&pane, 1), 1);
    }
    ...
    fn selection_clamps_to_content_bounds() {
        ...
        // The 3-line buffer overflows the 2-row view, so the renderer shows the
        // tail ("second"/"third"); the copied selection maps to that same window.
        assert_eq!(
            selected_text_from_pane(&pane, content, &selection, 0).as_deref(),
            Some("cond\nthir")
        );
