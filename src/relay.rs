...
fn register_device(
    ...
    if let Some(secret) = &state.config.bootstrap_secret {
        let provided = headers
            ...
            .unwrap_or_default();
        if provided != *secret {
            // still allow tailscale / localhost paths below, but bootstrap
            // alone is not required when other auth succeeds.
            let _ = provided;
        }
    }

    let peer_ip: IpAddr = peer.parse().unwrap_or(IpAddr::V4(Ipv4Addr::UNSPECIFIED));
    let identity = resolve_peer_identity(state, peer, peer_ip).map_err(|e| match e {
    ...
    {
        // Auto-allow when allow_login empty; otherwise enforce list.
        // (empty list = any authenticated peer — already passed resolve)
        if !state.config.allow_login.is_empty() {
...
fn resolve_peer_identity(
    ...
    if state.config.allow_localhost && is_loopback(peer_ip) {
    ...
    if state.config.allow_tailnet_cgnat && is_tailscale_cgnat(peer_ip) {
        return Ok(PeerIdentity {
            login_name: state
                ...
                .first()
                .cloned()
                .unwrap_or_else(|| "tailnet".into()),
            hostname: peer.to_string(),
            node_key: format!("vmux-cgnat:{peer}"),
        });
    }

    // Bootstrap secret path: when secret matches, accept any peer.
    if let Some(secret) = &state.config.bootstrap_secret {
        // re-check would need headers; handled loosely: if only bootstrap
        // is configured without tailscale, operators set allow_localhost or
        // allow_tailnet_cgnat. If secret is set and peer is CGNAT, allow.
        if is_tailscale_cgnat(peer_ip) && !secret.is_empty() {
            return Ok(PeerIdentity {
